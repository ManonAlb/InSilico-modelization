...
for file in files:
    if file.endswith(('.pdb')):
        ...
        wave = PlaneWave(energy=3e5, sampling=0.05)
        potential = Potential(system, parametrization='lobato', sampling=0.05, slice_thickness=2, projection='finite')
...
#KEEP IN MEMORY
        ...
        ax, im = block_zeroth_order_spot(pw_diffraction_pattern).show(power=0.2, cmap='jet') #blockin zero order for output image
        plt.savefig(diffraction_path + file.strip('.pdb') + '.png', dpi=300)
