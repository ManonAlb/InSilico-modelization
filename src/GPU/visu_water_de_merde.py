...
def diffracto(path, title, nmax):
    ...
    for file in files :
        ...
        if file.endswith(('.hdf5')) :
                ...
                alphax =(2*np.pi*( measurement.calibrations[-2].coordinates(measurement.array.shape[-2] ) ) )/2e-12
    ...
    plt.yscale('log')
    plt.plot(alphax, corrected)

    

    plt.savefig('diffractogram_{}'.format(path),dpi=300, bbox_inches='tight')



diffracto('Water', 'Water box for 1 ns, I in function of momentum transfer q', 1)

diffracto('Protein','Ubiquitin in vacuum for 1 ns, I in function of momentum transfer q', 1 )

#diffracto('Fixed_protein_in_water','Fixed ubiquitin in water for 1 ns, I in function of momentum transfer q', 1 ) 
