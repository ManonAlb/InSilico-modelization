...
# "REMOVE" THE FIRST ORDER BY HAND
...
# Function that will stack all the .hdf 5 files and generate diffractograms by integrating over a circle
...
def diffracto(path, title, nmax):
    ...
    for file in files :
        ...
        if file.endswith(('.hdf5')) :
                ...
                q = (2*np.pi*(measurement.calibrations[-2].coordinates(measurement.array.shape[-2])))/2e-12
                ...
                size = q.shape[0] #int(np.sqrt(i_size**2 + j_size**2))
    ...
    average = multi/c
    print(c) # number of files in your directory you have stacked alltogether
    ...
    return(plt.plot( q, corrected))


diffracto('Protein', 'I(q)', 1)
plt.show()
